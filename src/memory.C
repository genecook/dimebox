...
Memory::Memory() : block_size(PM_BLOCK_SIZE), physical_address_lo(0), physical_address_hi(0xffffffff) {
};
...
int Memory::ReadMemoryBlock(State *cpu,unsigned long long LA,bool is_data,
  ...
  Translation *trans2 = NULL;
  
  trans = mmu.LA2PA(LA,NS,privileged,is_data,is_aligned,number_of_bytes,false);
...
int Memory::WriteMemoryBlock(State *cpu,unsigned long long LA,bool is_data,
  ...
  trans = mmu.LA2PA(LA,NS,privileged,is_data,is_aligned,number_of_bytes,true);
...
bool Memory::AccessGlobalMonitor(MMU *mmu,Translation *trans) {
...
void Memory::SetExclusiveMonitor(State *cpu,unsigned long long logical_address,int num_bytes,bool privileged) {
  ...
  cpu->LocalMonitor().Set(logical_address,trans->PA(),num_bytes,cpu->GetID());
}
...
bool Memory::IsExclusive(State *cpu,unsigned long long logical_address,int num_bytes,bool privileged) {
  ...
  if (check_global_monitor) {
    ...
    // check local monitor...
    is_exclusive = cpu->LocalMonitor().IsExclusive(logical_address,trans->PA(),num_bytes,cpu->GetID());
  }
