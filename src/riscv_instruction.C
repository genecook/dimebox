...
//**************************************************************************
// return instruction class instance based on encoded instruction...
//**************************************************************************
...
RiscvInstruction * RiscvInstructionFactory::NewInstruction(RiscvState *state,Memory *memory,
							   unsigned int encoding,bool compressed) {

  // decode compressed extension instruction...
  
  if (compressed)
    return NewInstruction16(state,memory,encoding);
  ...
  //printf("# RV32 opcode: 0x%02x funct3: 0x%x rs1: 0x%x rs2: 0x%x funct7: 0x%02x imm: %0x03x\n",
...
//*****************************************************************************
// return compressed instruction class instance based on encoded instruction...
//*****************************************************************************

RiscvInstruction * RiscvInstructionFactory::NewInstruction16(RiscvState *state,Memory *memory,
							     unsigned int encoding) {

  //printf("[NewInstruction16] entered...\n");
  
  if (!state->CompressedExtensionEnabled())
    return INSTR_INST(UnknownInstruction);

  if (encoding == 0)
    return INSTR_INST(UnknownInstruction);
  
  if (encoding == 1)
    return INSTR_INST(C_NOP);

  RiscvInstruction *instruction = NULL;
  
  int funct3 = (encoding >> 13) & 0x7;
  int op = encoding & 0x3;
  int rd = (encoding >> 7) & 0x1f;
  
  //printf("# RVC opcode: funct3: 0x%x op: 0x%x rd: 0x%x\n",(unsigned) funct3,(unsigned) op,(unsigned) rd);
  
  switch( (funct3<<4) | op ) {
    case 0x00: instruction = INSTR_INST(C_ADDI4SPN); break;
    case 0x20: instruction = INSTR_INST(C_LW); break;
    case 0x60: instruction = INSTR_INST(C_SW); break;
    case 0x01: instruction = INSTR_INST(C_ADDI); break;
    case 0x11: instruction = INSTR_INST(C_JAL); break;
    case 0x21: instruction = INSTR_INST(C_LI); break;

    case 0x31: if (rd == 2)
	         instruction = INSTR_INST(C_ADDI16SP);
               else if (rd == 0)
                 instruction = INSTR_INST(UnknownInstruction);
	       else
		 instruction = INSTR_INST(C_LUI);
               break;
	       
    case 0x41: switch( (((encoding>>10) & 0x7)<<4) | ((encoding>>5) & 0x3) ) {
	         case 0x20:
	         case 0x21:
	         case 0x22:
	         case 0x23: instruction = INSTR_INST(C_ANDI); break;
	         case 0x30: instruction = INSTR_INST(C_SUB); break;
	         case 0x31: instruction = INSTR_INST(C_XOR); break;
	         case 0x32: instruction = INSTR_INST(C_OR); break;
	         case 0x33: instruction = INSTR_INST(C_AND); break;
	         default: instruction = INSTR_INST(UnknownInstruction); break;
               }
	       break;
	       
    case 0x51: instruction = INSTR_INST(C_J); break;
    case 0x61: instruction = INSTR_INST(C_BEQZ); break;
    case 0x71: instruction = INSTR_INST(C_LUI); break;
